import { FormComponentResetSymbol, UseFormUtils, config, createHeadManager, formDataToObject, getInitialPageFromDOM, getScrollableParent, isUrlMethodPair, mergeDataIntoQueryString, resetFormFields, router, router as router10, setupProgress, shouldIntercept, shouldNavigate, useInfiniteScroll } from "@inertiajs/core";
import { Fragment, computed, createSSRApp, defineComponent, h, markRaw, onBeforeUnmount, onMounted, onUnmounted, provide, reactive, ref, shallowRef, watch } from "vue";
...
import default2 from "@inertiajs/core/server";
	...
	const typedForm = reactive({
		...
		withPrecognition(...args2) {
			...
			validator.on("validatingChanged", () => {
				...
				const validationErrors = withAllErrors ?? config$1.get("form.withAllErrors") ? validator.errors() : toSimpleValidationErrors(validator.errors());
				this.errors = {};
		...
		submit(...args2) {
			...
			const _options = {
				...
				onSuccess: async (page2) => {
					...
					recentlySuccessfulTimeoutId = setTimeout(() => this.recentlySuccessful = false, config$1.get("form.recentlySuccessfulDuration"));
...
async function createInertiaApp({ id = "app", resolve, setup, title, progress: progress2 = {}, page: page2, render, defaults = {} }) {
	...
	const isServer = typeof window === "undefined";
	const useScriptElementForInitialPage = config$1.get("future.useScriptElementForInitialPage");
...
var link_default = defineComponent({
	...
	setup(props, { slots, attrs }) {
		...
		const cacheForValue = computed(() => {
			...
			return config$1.get("prefetch.cacheFor");
		});
		onMounted(() => {
		...
		const prefetchHoverEvents = {
			onMouseenter: () => {
				hoverTimeout.value = setTimeout(() => {
					prefetch();
				}, config$1.get("prefetch.hoverDelay"));
			},
			onMouseleave: () => {
				clearTimeout(hoverTimeout.value);
			},
...
//#endregion
export { router10 as a, link_default as i, createInertiaApp as n, useForm as o, head_default as r, default2 as t };
