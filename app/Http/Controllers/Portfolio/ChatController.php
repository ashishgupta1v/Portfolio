...
final class ChatController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if (! $apiKey) {
            ...
            $contactDirect = "You can contact Ashish Gupta directly via email at ashishgupta1v@gmail.com, download his résumé at /resume/ashish-gupta-resume.pdf, or view his hiring brief at /for-hiring-managers.";
            ...
            if (in_array($lastUserMessage, ['yes', 'sure', 'yes please', 'share links', 'send links', 'please do', 'yep', 'yeah', 'links']) || str_contains($lastUserMessage, 'share') || str_contains($lastUserMessage, 'send them')) {
                $reply = $linksList;
            } elseif (str_contains($lastUserMessage, 'contact') || str_contains($lastUserMessage, 'email') || str_contains($lastUserMessage, 'reach') || str_contains($lastUserMessage, 'hire') || str_contains($lastUserMessage, 'call')) {
                $reply = $contactDirect;
            ...
            } elseif (preg_match('/\b(ai|rag|vector|pgvector|llm|agent|gpt|zoeticoach)\b/i', $lastUserMessage) || str_contains($lastUserMessage, 'intelligence')) {
                $reply = "He builds *production* AI, not demos: a grounded, guarded RAG pipeline (OpenAI text-embedding-3-small, 1,536-dim vectors) with pgvector HNSW indexing (cosine distance) running live on WhatsApp for ZoetiCoach AI — sub-second retrieval (~780ms), XML envelope prompt-injection defense, an asynchronous Human-in-the-Loop Approval Queue for coach review, and an immutable Trust & Audit Log tracking every trace and token cost (~$0.018/client/day). Read the deep dive: /case-studies/zoeticoach-ai-whatsapp-accountability-engine." . $closeOffer;
...
- AI / RAG experience: \"He builds *production* AI, not demos: a grounded, guarded RAG pipeline (OpenAI text-embedding-3-small, 1,536-dim vectors) with pgvector HNSW indexing (cosine distance) running live on WhatsApp for ZoetiCoach AI — sub-second retrieval (~780ms), XML envelope prompt-injection defense, an asynchronous Human-in-the-Loop Approval Queue for coach review, and an immutable Trust & Audit Log tracking every trace and token cost (~$0.018/client/day). Case study: /case-studies/zoeticoach-ai-whatsapp-accountability-engine.\"
